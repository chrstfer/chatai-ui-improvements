...
Deno.test("unit: parseOrgInline creates single text object for plain text", () => {
    const nodes = parseOrgInline("Hello, this is pure text.");
    assertEquals((nodes[0] as OrgTextObject).value, "Hello, this is pure text.");
});

Deno.test("unit: parseOrgInline parses bold emphasis delimiters", () => {
...
Deno.test("unit: parseOrgInline parses italic emphasis delimiters", () => {
...
Deno.test("unit: parseOrgInline parses underline emphasis delimiters", () => {
...
Deno.test("unit: parseOrgInline parses strike emphasis delimiters", () => {
...
Deno.test("unit: parseOrgInline parses code monospace leaf node without inner markup", () => {
    const nodes = parseOrgInline("Run ~const x = *not_bold*;~ in terminal.");
    assertEquals((nodes[1] as OrgCodeObject).value, "const x = *not_bold*;");
});

Deno.test("unit: parseOrgInline parses verbatim monospace leaf node without inner markup", () => {
    const nodes = parseOrgInline("Check =/raw/ and *unparsed*= symbol.");
    assertEquals((nodes[1] as OrgVerbatimObject).value, "/raw/ and *unparsed*");
});

Deno.test("unit: parseOrgInline recursively parses nested emphasis markers", () => {
...
Deno.test("unit: parseOrgInline parses link with description", () => {
...
Deno.test("unit: parseOrgInline parses display double-dollar LaTeX fragment", () => {
    const nodes = parseOrgInline("Formula: $$\\sum_{i=1}^n x_i = S$$ is centered.");
    const math = nodes[1] as OrgLatexFragmentObject;
    assertEquals(math.isDisplay, true);
});

Deno.test("unit: parseOrgInline parses display bracket LaTeX fragment", () => {
    const nodes = parseOrgInline("Equation: \\[\\int_0^1 f(x)dx\\] is definite.");
    const math = nodes[1] as OrgLatexFragmentObject;
    assertEquals(math.isDisplay, true);
});

Deno.test("unit: parseOrgInline parses fraction statistics cookie", () => {
...
Deno.test("unit: parseOrgInline parses percent statistics cookie", () => {
    const nodes = parseOrgInline("Progress is [75%].");
    const cookie = nodes[1] as OrgStatisticsCookieObject;
    assertEquals(cookie.percent, 75);
});

Deno.test("unit: parseOrgInline parses forced line break", () => {
    const nodes = parseOrgInline("First line\\\\\nSecond line");
    assertEquals(nodes[1].type, "line_break");
});

Deno.test("unit: parseOrgInline does not treat arithmetic multiplication asterisks as bold", () => {
    const nodes = parseOrgInline("5 * 4 * 3 = 60");
    assertEquals(nodes.length, 1);
});

Deno.test("unit: parseOrgInline rejects emphasis delimiters with leading inner whitespace", () => {
    const nodes = parseOrgInline("* not bold*");
    assertEquals(nodes.length, 1);
});

Deno.test("unit: parseOrgInline rejects emphasis delimiters with trailing inner whitespace", () => {
    const nodes = parseOrgInline("*not bold *");
    assertEquals(nodes.length, 1);
});

Deno.test("unit: parseOrgInline rejects mid-word asterisks", () => {
